...
namespace BE
{
	...
	void ContentBrowserPanel::OnImGuiRender()
	{
		...
		if (bShowInputFolderName)
		{
			...
			UI::ButtonType pressedButton = UI::InputPopup("BunnyEngine-Editor", "Folder name", input);
		...
		{
			UI::ButtonType result = UI::ShowMessage("BunnyEngine-Editor", "Do you want to save current scene?", UI::ButtonType::YesNoCancel);
	...
	void ContentBrowserPanel::DrawContent(const std::vector<std::filesystem::path>& directories, const std::vector<std::filesystem::path>& files, bool bHintFullPath /* = false */)
	{
		...
		for (auto& dir : directories)
		{
			...
			bHoveredAnyItem |= ImGui::IsItemHovered();
			if (ImGui::IsItemHovered() && GImGui->HoveredIdTimer > BE_HOVER_THRESHOLD)
			{
				ImGui::BeginTooltip();
