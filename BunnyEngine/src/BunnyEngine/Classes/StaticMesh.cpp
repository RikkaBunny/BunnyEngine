...
namespace BE
{
	...
	std::vector<Ref<Texture2D>> loadMaterialTextures(aiMaterial* mat, aiTextureType type, const std::filesystem::path& filename)
	{
		...
		for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
		{
			...
			BE_CORE_TRACE("SM Texture Path: {0}", absolutePath.u8string());
	...
	Ref<StaticMesh> StaticMesh::Create(const std::filesystem::path& filename, bool bLazy /* = false */, bool bForceImportingAsASingleMesh /* = false */, bool bAskQuestion /* = true */)
	{
		...
		{
			BE_CORE_ERROR("Failed to load Static Mesh. File doesn't exist! ({0})", filename);
			return nullptr;
		}
		...
		{
			BE_CORE_ERROR("Failed to load Static Mesh. {0} ({1})", importer.GetErrorString(), filename);
			return nullptr;
		}
		...
		if (meshesCount == 0)
		{
			BE_CORE_ERROR("Failed to load Static Mesh. No meshes in file '{0}'", filename);
			return nullptr;
		}
		...
		if (meshesCount > 1)
		{
			if (bForceImportingAsASingleMesh || (bAskQuestion && Dialog::YesNoQuestion("BunnyEngine-Editor", "Importing file contains multiple meshes.\nImport all meshes as a single mesh?")))
			{
				std::vector<Vertex> vertices;
