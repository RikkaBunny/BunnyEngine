...
namespace BE
{
	...
	void Sound::Play()
	{
		if (AudioEngine::PlaySound(m_Sound, &m_Channel))
		{
			...
			if (res != FMOD_OK)
				BE_CORE_WARN("[AudioEngine] Failed to set pan. Path: {0}. Error: {1}", m_SoundPath, FMOD_ErrorString(res));
		}
		else
			BE_CORE_WARN("[AudioEngine] Failed to play sound. Path: {0}. Error: {1}", m_SoundPath);
	}
	...
	void Sound::Stop()
	{
		if (IsPlaying())
		{
			...
			if (res != FMOD_OK)
				BE_CORE_WARN("[AudioEngine] Failed to stop sound. Path: {0}. Error: {1}", m_SoundPath, FMOD_ErrorString(res));
		}
	}
	...
	void Sound::SetPaused(bool bPaused)
	{
		if (m_Channel)
		{
			...
			if (res != FMOD_OK)
				BE_CORE_WARN("[AudioEngine] Failed to call 'SetPaused'. Path: {0}. Error: {1}", m_SoundPath, FMOD_ErrorString(res));
		}
		else
			BE_CORE_WARN("[AudioEngine] Can't call 'SetPaused'. Sound wasn't initialized by calling Sound::Play(). Path: {0}", m_SoundPath);
	}
	...
	void Sound::SetPosition(uint32_t ms)
	{
		if (m_Channel)
		{
			...
			if (res != FMOD_OK)
				BE_CORE_WARN("[AudioEngine] Failed to set sound position. Path: {0}. Error: {1}", m_SoundPath, FMOD_ErrorString(res));
		}
		else
			BE_CORE_WARN("[AudioEngine] Can't set sound position. Sound wasn't initialized by calling Sound::Play(). Path: {0}", m_SoundPath);
	}
