...
namespace BE
{
	...
	void AudioEngine::Init(const AudioEngineSettings& settings)
	{
		...
		if (result != FMOD_OK)
		{
			BE_CORE_CRITICAL("[AudioEngine] Failed to init Audio System. Error: {0}", FMOD_ErrorString(result));
			BE_CORE_ASSERT(false, "Audio init failure");
		}
	}
	...
	bool AudioEngine::CreateSound(const std::filesystem::path& path, uint32_t playMode, FMOD::Sound** sound)
	{
		...
		{
			BE_CORE_ERROR("[AudioEngine] Failed to create sound. Filepath doesn't exist: {0}", absolutePath);
			return false;
		}
		...
		if (it != s_LoadedSounds.end())
		{
			...
			{
				bSuccess = false;
				BE_CORE_ERROR("[AudioEngine] Failed to create sound. Audio filepath: {0}. Error: {1}", path, FMOD_ErrorString((FMOD_RESULT)res));
			}
		}
		else
		{
			...
			{
				bSuccess = false;
				BE_CORE_ERROR("[AudioEngine] Failed to create sound. Audio doesn't exist. Path: {0}", path);
			}
		}
	...
	bool AudioEngine::CreateReverb(FMOD::Reverb3D** reverb)
	{
		...
		if (res != FMOD_OK)
		{
			BE_CORE_ERROR("[AudioEngine] Failed to create reverb. Error: {0}", FMOD_ErrorString((FMOD_RESULT)res));
			return false;
		}
		return true;
	}
