...
namespace BE
{
	...
	{
		BE_CORE_ASSERT(!s_PhysXData, "Trying to init physics twice!");
		...
		BE_CORE_ASSERT(s_PhysXData->Foundation, "Failed to create foundation");
		...
		BE_CORE_ASSERT(s_PhysXData->Physics, "Failed to create Physics");

		bool bExtensionsLoaded = PxInitExtensions(*s_PhysXData->Physics, PhysXDebugger::GetDebugger());
		BE_CORE_ASSERT(bExtensionsLoaded, "Failed to init Extensions");
	...
	void PhysicsErrorCallback::reportError(physx::PxErrorCode::Enum code, const char* message, const char* file, int line)
	{
		...
		switch (code)
		{
			...
			case physx::PxErrorCode::Enum::eDEBUG_INFO:
				BE_CORE_INFO("[Physics Engine]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
				break;
			case physx::PxErrorCode::Enum::eDEBUG_WARNING:
			case physx::PxErrorCode::Enum::ePERF_WARNING:
				BE_CORE_WARN("[Physics Engine]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
				break;
			...
			case physx::PxErrorCode::Enum::eINTERNAL_ERROR:
				BE_CORE_ERROR("[Physics Engine]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
				break;
			case physx::PxErrorCode::Enum::eABORT:
			case physx::PxErrorCode::Enum::eMASK_ALL:
				BE_CORE_CRITICAL("[Physics Engine]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
				BE_CORE_ASSERT(false, "Critical error");
				break;
		}
	}
	...
	{
		BE_CORE_ERROR("[Physics Engine]: {0}: at {1} ({2})", exception, file, line);
	}
}
