...
#include "BunnyEngine/Core/DataBuffer.h"
...
namespace BE
{
	...
	{
		BE_CORE_ASSERT(!s_CookingData, "Trying to init Cooking Factory twice!");
		...
		BE_CORE_ASSERT(s_CookingData->CookingSDK, "Failed to create Cooking");
	}
	...
	CookingResult PhysXCookingFactory::CookMesh(MeshColliderComponent& component, bool bInvalidateOld, MeshColliderData& outData)
	{
		...
		if (!collisionMesh)
		{
			BE_CORE_ERROR("[Physics Engine] Cooking: Invalid mesh!");
		...
		if (bInvalidateOld)
		{
			...
			if (!removedCached)
				BE_CORE_ERROR("Couldn't delete cached collider data: '{0}'", filepath.u8string());
		}
		...
		if (!std::filesystem::exists(filepath))
		{
			...
			if (result == CookingResult::Success)
			{
				...
				if (!bSuccessWrite)
					BE_CORE_ERROR("Failed to write collider to '{0}'", filepath.u8string());
			}
		}
		else
		{
	...
	CookingResult PhysXCookingFactory::CookConvexMesh(const Ref<StaticMesh>& mesh, MeshColliderData& outData)
	{
		...
		{
			BE_CORE_ERROR("[Physics Engine] Failed to cook convex mesh '{0}'", mesh->GetPath());
	...
	CookingResult PhysXCookingFactory::CookTriangleMesh(const Ref<StaticMesh>& mesh, MeshColliderData& outData)
	{
		...
		triangleDesc.triangles.data = &indices[0];
				...
				if (!bValid)
				{
					BE_CORE_ERROR("[Physics Engine] Failed to validate triangle mesh '{0}'", mesh->GetPath());
					return CookingResult::Failure;
				}
		#endif
		...
		{
			BE_CORE_ERROR("[Physics Engine] Failed to cook triangle mesh '{0}'", mesh->GetPath());
			return PhysXUtils::FromPhysXCookingResult(result);
		}
