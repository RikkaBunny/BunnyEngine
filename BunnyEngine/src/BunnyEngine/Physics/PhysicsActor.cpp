...
namespace BE
{
	...
	void PhysicsActor::SetMass(float mass)
	{
		if (!IsDynamic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot set mass of non-dynamic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return;
		}

		physx::PxRigidDynamic* actor = m_RigidActor->is<physx::PxRigidDynamic>();
		BE_CORE_ASSERT(actor, "No actor");
	...
	void PhysicsActor::AddForce(const glm::vec3& force, ForceMode forceMode)
	{
		if (!IsDynamic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot add force to non-dynamic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return;
		}
		else if (m_RigidBodyComponent.IsKinematic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot add force to Kinamatic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return;
		}
	...
	void PhysicsActor::AddTorque(const glm::vec3& torque, ForceMode forceMode)
	{
		if (!IsDynamic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot add torque to non-dynamic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return;
		}
		else if (m_RigidBodyComponent.IsKinematic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot add torque to Kinamatic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return;
		}
	...
	glm::vec3 PhysicsActor::GetLinearVelocity() const
	{
		if (!IsDynamic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot get linear velocity of non-dynamic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return glm::vec3(0.f);
		}
	...
	void PhysicsActor::SetLinearVelocity(const glm::vec3& velocity)
	{
		if (!IsDynamic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot set linear velocity of non-dynamic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return;
		}
	...
	glm::vec3 PhysicsActor::GetAngularVelocity() const
	{
		if (!IsDynamic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot get angular velocity of non-dynamic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return glm::vec3(0.f);
		}
	...
	void PhysicsActor::SetAngularVelocity(const glm::vec3& velocity)
	{
		if (!IsDynamic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot set angular velocity of non-dynamic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return;
		}
	...
	float PhysicsActor::GetMaxLinearVelocity() const
	{
		if (!IsDynamic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot get max linear velocity of non-dynamic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return 0.f;
		}
	...
	void PhysicsActor::SetMaxLinearVelocity(float maxVelocity)
	{
		if (!IsDynamic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot set max linear velocity of non-dynamic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return;
		}
	...
	float PhysicsActor::GetMaxAngularVelocity() const
	{
		if (!IsDynamic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot get max angular velocity of non-dynamic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return 0.f;
		}
	...
	void PhysicsActor::SetMaxAngularVelocity(float maxVelocity)
	{
		if (!IsDynamic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot set max angular velocity of non-dynamic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return;
		}
	...
	void PhysicsActor::SetLinearDamping(float damping) const
	{
		if (!IsDynamic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot set linear damping of non-dynamic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return;
		}
	...
	void PhysicsActor::SetAngularDamping(float damping) const
	{
		if (!IsDynamic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot set angular damping of non-dynamic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return;
		}
	...
	glm::vec3 PhysicsActor::GetKinematicTargetLocation() const
	{
		if (!IsKinematic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot get kinematic target location of non-kinematic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return glm::vec3(0.f);
		}
	...
	Rotator PhysicsActor::GetKinematicTargetRotation() const
	{
		if (!IsKinematic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot get kinematic target rotation of non-kinematic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return glm::vec3(0.f);
		}
	...
	void PhysicsActor::SetKinematicTarget(const glm::vec3& location, const Rotator& rotation)
	{
		if (!IsKinematic())
		{
			BE_CORE_WARN("[PhysicsEngine] Cannot set kinematic target of non-kinematic PhysicsActor. Entity: '{0}'", m_Entity.GetSceneName());
			return;
		}
	...
	bool PhysicsActor::SetKinematic(bool bKinematic)
	{
		if (!IsDynamic())
		{
			BE_CORE_WARN("[PhysicsEngine] Static PhysicsActor can't be kinematic. Entity: '{0}'", m_Entity.GetSceneName());
			return false;
		}
	...
	bool PhysicsActor::SetLockFlag(ActorLockFlag flag, bool value)
	{
		if (!IsDynamic())
		{
			BE_CORE_WARN("[PhysicsEngine] Can't lock Static Physics Actor. Entity: '{0}'", m_Entity.GetSceneName());
			return false;
		}
	...
	Ref<MeshShape> PhysicsActor::AddCollider(MeshColliderComponent& collider)
	{
		...
		if (!collisionMesh)
		{
			BE_CORE_ERROR("[Physics Engine] Set collision mesh inside MeshCollider Component. Entity: '{0}'", collider.Parent.GetSceneName());
		...
		else
		{
			...
			{
				BE_CORE_ERROR("[Physics Engine] Can't have a non-convex MeshColliderComponent for a non-kinematic dynamic RigidBody Component. Entity: '{0}'", m_Entity.GetSceneName());
				return s_InvalidCollider;
			}
