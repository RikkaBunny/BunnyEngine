...
#include "BunnyEngine/Camera/CameraController.h"
#include "BunnyEngine/Renderer/Shader.h"
#include "BunnyEngine/Physics/PhysicsMaterial.h"
...
namespace YAML
{
	...
	struct convert<glm::quat>
	{
		...
		static bool decode(const Node& node, glm::quat& rhs)
		{
			...
			if (node.size() == 3)
			{
				...
				rhs = BE::Rotator::FromEulerAngles(euler).GetQuat();
				return true;
			}
			else if (node.size() == 4)
			{
	...
	template<>
	struct convert<BE::Rotator>
	{
		static Node encode(const BE::Rotator& rhs)
		{
			Node node;
		...
		static bool decode(const Node& node, BE::Rotator& rhs)
		{
			if (!node.IsSequence())
				return false;
			...
			if (node.size() == 3)
			{
				...
				rhs = BE::Rotator::FromEulerAngles(euler);
				return true;
			}
			else if (node.size() == 4)
			{
...
namespace BE
{
	...
	{
		BE_CORE_TRACE("Saving Scene at '{0}'", std::filesystem::absolute(filepath));

		YAML::Emitter out;
		out << YAML::BeginMap;
		out << YAML::Key << "Scene"	<< YAML::Value << "Untitled";
		out << YAML::Key << "Version" << YAML::Value << BE_VERSION;
	...
	bool SceneSerializer::Deserialize(const std::filesystem::path& filepath)
	{
		...
		{
			BE_CORE_WARN("Can't load scene {0}. File doesn't exist!", std::filesystem::absolute(filepath));
			return false;
		}
		...
		if (!data["Scene"])
		{
			BE_CORE_WARN("Can't load scene {0}. File has invalid format!", std::filesystem::absolute(filepath));
			return false;
		}
		BE_CORE_TRACE("Loading scene '{0}'", std::filesystem::absolute(filepath));
	...
	{
		BE_CORE_ASSERT(false, "Not supported yet");
		return false;
	}
