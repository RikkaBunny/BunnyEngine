...
namespace BE
{
	...
	{
		BE_CORE_ASSERT(std::filesystem::exists(filepath), "Shader was not found!");
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::Preprocess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			BE_CORE_ASSERT(nextLinePos != std::string::npos, "Syntax error");
	...
	void OpenGLShader::CompileAndLink(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		BE_CORE_ASSERT(shaderSources.size() <= SupportsShadersNum, "BunnyEngine supports only 3 shaders for now!");
	...
	{
		BE_CORE_ASSERT(std::filesystem::exists(m_Path), "Shader was not found!");
		FreeMemory();
	...
	void OpenGLShader::SetInt(const std::string& name, int value)
	{
		int location = glGetUniformLocation(m_RendererID, name.c_str());
			...
			if (location == -1)
			{
				BE_CORE_WARN("Did not found uniform '{0}' in shader '{1}'", name, m_Path);
				return;
			}
		#endif 
