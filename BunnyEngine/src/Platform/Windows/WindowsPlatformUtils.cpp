...
namespace BE
{
	...
	namespace FileSystem
	{
		...
		DataBuffer Read(const std::filesystem::path& path)
		{
			...
			BE_CORE_ASSERT(size != 0, "Empty file");
	...
	namespace Utils
	{
		...
		{
			static HANDLE bunnyEvent = CreateEventA(NULL, false, false, "BunnyEngine-Editor");
			if (bunnyEvent == 0)
				return false;

			return WaitForSingleObject(bunnyEvent, 0) == WAIT_OBJECT_0;
		}
	}
